...
- **Shared previews (2026-07-05)**: `src/lib/previs.js` (`window.Previs`, inlined via `PrevisEngine.astro` — same strip pipeline + no-`//`-in-string/regex-literals constraint as patch-runtime.js) owns all work_index viewer scenes: ALL 11 projects have a `V`-registry entry — 5 featured live previews + 6 bespoke recreated minis (equipment board, orb, animatronic eye, lighting floorplan, BEAM plant, sensor-zone room; vfoot label starts "RECREATION"). Homepage index = 5 featured + see-more row → /projects; /projects = the full-index network (`work_txt → work_index ─selection→ projects_view`; compact single-line rows, LIVE/RECR badges, no category filter — Carlton cut it 2026-07-05), READ mode reuses the homepage sticky-viewer rack. Navbar `/work` → `/projects` (prefix-aware active state); home is reachable via the brand mark. New previews go in previs.js's `V` registry, not in page scripts.
