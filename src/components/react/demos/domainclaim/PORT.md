...
| `DomainClaimFrame.jsx`, `SignInHero.jsx`, `FailureExplorer.jsx` | The case study's side: the scaled frame and the two VIEWs. |
...
- `runtime/FrameApp.tsx` mirrors the product's root and app layouts, and takes one command from the
  case study: open a demo name as a fresh claim (optionally held by a second account first).
